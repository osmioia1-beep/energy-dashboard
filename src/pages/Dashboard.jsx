    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-3 gap-3">
        <div className="bg-gray-900 rounded-xl p-3 text-center border border-gray-800">
          ...
          <p className="text-lg font-bold text-white">{totalPower.toFixed(0)}</p>
          <p className="text-xs text-gray-500">Watts</p>
        </div>
        <div className="bg-gray-900 rounded-xl p-3 text-center border border-gray-800">
          <Activity className="w-5 h-5 text-green-500 mx-auto mb-1" />
          <p className="text-lg font-bold text-white">{activeDevices}</p>
      ...
      <div className="flex items-center justify-between">
        <h2 className="text-sm font-semibold text-gray-400 uppercase tracking-wider">Dispositivos</h2>
        <button
            ...
            {recentEvents.slice(0, 8).map((event, i) => (
              <div key={i} className="flex items-center justify-between px-4 py-3">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-sm font-medium text-white">{event.shelly_name || '—'}</p>
                    <p className="text-xs text-gray-500">{event.event_type}</p>
                  </div>
                </div>
                <div className="text-right">
                  <p className="text-sm text-white">{event.power_watts?.toFixed(0) || '—'} W</p>
