  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center gap-3">
        ...
        <h1 className="text-2xl font-bold text-white">V2C Trydan</h1>
      </div>
      ...
      {stats && (
        <div className="grid grid-cols-2 gap-3">
          ...
          <div className="bg-gray-900 rounded-xl p-4 border border-gray-800">
            ...
            <p className="text-xl font-bold text-white">{formatEnergy(stats.totalEnergyWh)}</p>
          </div>
          <div className="bg-gray-900 rounded-xl p-4 border border-gray-800">
            <div className="flex items-center gap-2 mb-2">
            ...
            <p className="text-xl font-bold text-white">{formatDuration(stats.totalDurationS)}</p>
          </div>
          <div className="bg-gray-900 rounded-xl p-4 border border-gray-800">
            <div className="flex items-center gap-2 mb-2">
            ...
            <p className="text-xl font-bold text-white">{stats.avgPower.toFixed(0)} W</p>
          </div>
        </div>
      )}
          ...
          <div key={session.id} className="bg-gray-900 rounded-xl p-4 border border-gray-800">
            <div className="flex items-center justify-between mb-2">
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm font-medium text-white">{session.carro || 'Carro'}</span>
              </div>
            ...
            <div className="grid grid-cols-3 gap-2 text-center">
              <div>
                ...
                <p className="text-sm font-semibold text-white">{formatEnergy(session.total_energy_wh)}</p>
              </div>
              <div>
                <p className="text-xs text-gray-500">Duração</p>
                <p className="text-sm font-semibold text-white">{formatDuration(session.duration_seconds)}</p>
              </div>
              <div>
            ...
            {session.max_power_watts && (
              <div className="mt-2 pt-2 border-t border-gray-800 text-center">
                <p className="text-xs text-gray-500">Potência Máxima</p>
                <p className="text-sm font-semibold text-yellow-400">{session.max_power_watts.toFixed(0)} W</p>
              </div>
            )}
          </div>
        ))}
